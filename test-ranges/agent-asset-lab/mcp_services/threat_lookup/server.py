...
app = FastAPI(title="Threat Lookup MCP")
class Tool:
...
def lookup(args): return {"source":"internet-stub","advisory":args.get("id","clean")}
TOOLS=[Tool("lookup","Look up a public threat advisory",lookup)]
@app.get("/health")
def health(): return {"status":"ok","server":"threat-lookup"}
@app.post("/mcp")
def mcp(body:dict):
    method=body.get("method"); params=body.get("params",{})
    if method=="initialize": result={"serverInfo":{"name":"threat-lookup"}}
